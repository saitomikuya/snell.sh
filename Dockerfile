...
# Colima runs natively on arm64; keeping the toolchain stages on the native
# platform avoids Go crashes under amd64 emulation while still cross-compiling
# the final binary for TARGETOS/TARGETARCH.
...
FROM --platform=$BUILDPLATFORM golang:1.25.0-bookworm AS go-build
WORKDIR /src
# Limit Go's parallel downloader/compiler workers for stable cross-platform
# builds under QEMU (the production binary remains unchanged).
ENV GOMAXPROCS=2
COPY go.mod go.sum ./
RUN go mod download
COPY . .
...
# Copy build outputs before installing runtime packages so BuildKit does not
# run the two amd64 emulated apt jobs concurrently on an arm64 builder.
...
ENV PANEL_DATA_DIR=/data PANEL_ANYCONNECT_RUNTIME_DIR=/run/proxy-panel/anyconnect PANEL_BIND=0.0.0.0 PANEL_PORT=8080 PANEL_UID=10001 PANEL_GID=10001 PANEL_AUTO_CREATE_TUN=1 TZ=Asia/Shanghai
VOLUME ["/data"]
EXPOSE 8080
