...
func (s *Service) RefreshCIDRs(ctx context.Context, node nodes.Node) (result AssetResult, returnErr error) {
	...
	content, err := download(ctx, s.client, node.Config.ChinaCIDRSourceURL, "", "")
	if err != nil {
	...
	return AssetResult{OK: true, Changed: changed, Kind: "cidr", Fingerprint: fp, Count: len(prefixes), UpdatedAt: succeeded, Message: fmt.Sprintf("已加载 %d 条中国大陆 IPv4 路由", len(prefixes))}, nil
}
