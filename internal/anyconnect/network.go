...
func download(ctx context.Context, client *http.Client, rawURL, username, password string) ([]byte, error) {
	...
	if response.ContentLength > maxAssetDownload {
		return nil, errors.New("下载内容超过 8 MiB 限制")
	}
	content, err := io.ReadAll(io.LimitReader(response.Body, maxAssetDownload+1))
	if err != nil {
		return nil, err
	}
	if len(content) == 0 || len(content) > maxAssetDownload {
