...
func (s *Service) PrepareConfig(ctx context.Context, node nodes.Node) (string, error) {
	...
	// The agent runs as root, but the panel server must be able to read this
	// tree when creating a consistency backup before an import.
	if err = repairPersistentTrees(target, lastGood); err != nil {
		return "", err
	}
...
func (s *Service) renderConfig(node nodes.Node, finalConfigDir string) ([]byte, error) {
	...
	for _, path := range []string{filepath.Dir(filepath.Dir(runtimeDir)), filepath.Dir(runtimeDir), runtimeDir} {
