...
| M4 网络高级功能 | 部分完成 | 专属 nftables 表、节点/项目字节采样、AnyConnect 账号级 occtl 流量采样与独立限额、AnyConnect NAT、Docker `FORWARD` 兼容及中国 CIDR 定时原子更新完成；更完整宿主机指标待补 |
...
- AnyConnect 账号级 occtl RX/TX 采样、周期累计、限额状态和超限会话终止已接入 Agent；ocserv 离线时保留数据库中的上次累计值。
