...
勾选“下发可选服务器列表”可生成 `profile.xml`：每行填写 `显示名称 | 域名:端口`，当前节点会自动加入第一项。用户首次成功连接后，Cisco Secure Client 会自动下载并保存该文件；多台独立部署的面板只需手动填写相同列表即可保持客户端的节点列表一致；它不会改变用户路由组。若手机提示无法下载配置文件，请重新保存并应用节点，再查看节点日志中的 `cannot load config file` 或 `error sending file`。
