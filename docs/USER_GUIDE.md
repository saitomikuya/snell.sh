...
勾选“下发可选服务器列表”可生成 `profile.xml`：每行填写 `显示名称 | 域名:端口`，当前节点会自动加入第一项。多台独立部署的面板只需手动填写相同列表即可保持 Cisco Secure Client 的节点列表一致；它不会改变用户路由组。
...
在“更新与备份 → 跨服务器同步”点击“导出当前节点及用户”，会下载普通 JSON 文件。文件包含节点密钥、AnyConnect 证书下载凭据和用户密码，按密码级别保护，传输完成后应立即删除或放入受保护的存储。

在另一台面板选择该 JSON 并点击“导入并合并”即可同步。导入前会自动创建本机备份；同 ID 优先匹配，其次按“类型 + 名称”匹配，缺少的节点会创建，默认不会删除目标节点。AnyConnect 用户按用户名更新或创建；勾选“删除目标 AnyConnect 中未出现在文件里的用户”才会执行用户删除。ShadowTLS 后端引用会随节点 ID 映射。若导入应用失败，请使用界面中提示的导入前备份恢复。
