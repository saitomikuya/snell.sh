...
| 节点 | 默认监听 | 是否对公网开放 |
|---|---|---|
| Snell 主节点 | `127.0.0.1:6160/tcp` | 否，作为 ShadowTLS 后端 |
| Snell ShadowTLS | `0.0.0.0:8443/tcp` | 是 |
| SS-2022 主节点 | 首次启动时选择可用端口，TCP+UDP | 是 |
...
客户端只连接 ShadowTLS 的公网端口。Snell 的 `127.0.0.1:6160` 不应对公网开放。
...
作为 ShadowTLS 后端时使用 `127.0.0.1`。只有明确希望直接公开 Snell 时才使用 `0.0.0.0`，并放行对应 TCP 端口。
...
进入“更新与备份”，点击“检查上游版本”。面板会比较：

- `jinqians/snell.sh` 和 `jinqians/ss-2022.sh` 的上游提交；
- Snell、shadowsocks-rust、ShadowTLS 的官方版本；
- 本地适配器声明的兼容版本。

“待适配”只表示发现上游变化，不能直接套用远程脚本。“可更新”才会出现“备份并更新”按钮。
...
- “节点 → 日志”：查看协议进程最近 200 行输出，常用于定位配置、端口和运行时问题；
- “日志与审计”：记录登录、改密、节点变更、启停、流量操作、更新、备份和恢复；
- “系统设置”：修改管理员密码。改密会立即注销所有会话；
